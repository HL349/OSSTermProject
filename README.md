...
가격표 사진에서 가격 정보를 추출하는 과정에서 글자 인식이 되긴 하나 인식률이 매우 낮게 나온 어려움이 있었다.
웹페이지와 만든 서비스의 연결, 즉 프런트와 백의 연결에 문제가 있어 직접적인 연결을 못하고 각 모듈의 결과 저장 후 그 저장된 값을 웹페이지로 불러오게끔 설계를 할 수밖에 없었다.
